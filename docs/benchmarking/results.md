...
> This is a recovery benchmark. Throughput is included as raw context, but it is not a useful capacity metric. The latency columns measure attempt-one start to attempt-two start (the crash-to-reclaim gap).
...
> This is a recovery benchmark. Read recovery duration, lease timing, and correctness—not jobs/sec—as its result. The latency columns are end-to-end job latency for the small recovery workload.
...
> Each point is a short burst containing only `concurrency` requests against one shared key. At the smallest sizes, startup and scheduling noise dominate, so these points verify contention safety and latency; they are not a monotonic service-scaling curve.
...
concurrency 1->2: 184->292 jobs/sec (+59%); concurrency 2->4: 292->416 jobs/sec (+42%); concurrency 4->8: 416->493 jobs/sec (+19%); concurrency 8->16: 493->369 jobs/sec (-25%)

Marginal throughput gain from added concurrency drops below 15% past **worker concurrency 8** in one worker process at 0ms downstream latency on this machine — consistent with claim/finalize transaction overhead (not the downstream) becoming the bottleneck. Treat this as a local, 8-logical-CPU-laptop number, not a universal ceiling.
...
These backlog numbers are not directly comparable to the `execution` scenario's 493 jobs/sec. The backlog points contain up to 2000 measured jobs, skip the execution scenario's resource samplers, and amortize process startup and polling over a larger drain. The execution sweep contains up to 400 measured jobs per point and additional resource instrumentation. Both use the real release worker binary, but they answer different workload questions.
...
CPU values come from 500ms `ps` samples of individual macOS processes; PostgreSQL CPU comes from Docker container sampling. They are directional utilization signals with different accounting boundaries, not a complete host-wide CPU budget. Short runs may contain too few samples to characterize peaks reliably.

### Idempotency-contention interpretation

The throughput points are non-monotonic (notably the concurrency-4 result). Each point issues only as many requests as its concurrency, so the smallest runs are millisecond-scale bursts dominated by scheduling, connection reuse, and timer noise. Use this scenario to evaluate the invariant—one committed charge per shared key—and its latency under contention. A sustained, fixed-request-count workload is required before making idempotency-service capacity claims.
...
- **The idempotency scenario uses microbursts, not a sustained load** — each point sends only `concurrency` requests. Its non-monotonic throughput values should not be interpreted as a scaling curve; the useful results are contention latency and the exactly-one-charge invariant.

## Charts

![Throughput vs concurrency in one worker process](../../benchmarks/reports/throughput_vs_concurrency.svg)
